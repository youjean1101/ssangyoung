...
//springweb.a06_util.Z01_DownloadView
public class A04_DownloadView extends AbstractView{
	//다운로드할 특정한 폴드 공통 위치
	@Value("${user.upload}")
	...
	protected void renderMergedOutputModel(Map<String, Object> model, 
		...
		//		model.adddAttribute("downloadFile",파일명)
		//	1) 파일명 선언
		...
		// 3. FileInputStream으로 파일 객체 탑재, response객체의 OutputStream으로
		//		스트림으로 client에 보내기
		FileInputStream fis = new FileInputStream(file);
		OutputStream out = response.getOutputStream();
		//	1) FilerCopyUtils를 통해 파일 객체를 outstream에 할당.
