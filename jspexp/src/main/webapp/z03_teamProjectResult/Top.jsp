								    	  ...
								    	  <c:forEach var="pd" items="${al.getAlertList(alert)}">
								    		<div class="tip">
									    		<ul>
										    		...
										    		<li>문의번호:${pd.qnano}<br>								    		
										    		<li><button type="button" onclick="location.href='${pd.moveurl}'">이동하기</button> 
										    		<button type="button" onclick="deletealert('${pd.alertno}')">삭제</button> 
									    		</ul>
								    		</div>
								    		</c:forEach>
...
function deletealert(val){
	location.href="CheckAlert.jsp?alertno="+val
}
