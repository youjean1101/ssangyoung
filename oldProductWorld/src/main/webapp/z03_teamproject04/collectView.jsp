		...
		<form id="collectRemoveform" action="collectRemove.jsp"><!--   -->
			...
			<table id="collectTab">
				...
				<%for(Olddealuser collectuserinfo:socialDao.cutoutView(new Social(Login.getId(),"모아"))){ %>
					<div class="flex-item">
							<div id="checkbox"><input type="checkbox" name="removeUserSel" value="<%=collectuserinfo.getId() %>"/></div>
							<div id="no"><%=i++ %></div>
							<div id="collid"><%=collectuserinfo.getId() %></div>
							<div id="address"><%=collectuserinfo.getAddress()%></div>
							<hr>
					</div>
