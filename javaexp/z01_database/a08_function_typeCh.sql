	...
	2) 명시적 데이터 타입의 변환
		...
		ex) 현재 컬럼 데이터 유형을 변환해서 조건에 맞게 처리하는 경우
			where to_char(hiredate, 'YYYY') = '2022'
				==>  날짜형인 데이터를 문자형날짜만 추출하여 비교하여 검색
			해당 컬럼에 맞게 데이터 유형을 변환해서 처리하는 경우
				where hiredate = to_date('2022-01-12','YYYY-MM-DDD')
				insert emp(hiredate) values(to_date('2022-01-12','YYYY-MM-DDD'))
