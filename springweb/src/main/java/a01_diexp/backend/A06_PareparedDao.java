...
public class A06_PareparedDao {
	...
	public void insertSalgrade(Salgrade ins) { 
		...
		try { 
			con = DB.con();
			con.setAutoCommit(false); 
			pstmt = con.prepareStatement(sql); // values('아이디', '패스워드', '이름', '권한', 1000, '주소') 
			pstmt.setString(1, ins.getGrade());
			pstmt.setDouble(2,ins.getLosal()); pstmt.setDouble(3,ins.getHisal());
...
public boolean chkDept(int deptno,String dname){
   ...
   public Member login(Member m){
	...
	// 조회 처리
   public List<Member> indexMemberList(Member sch){
      List<Member> dlist = new ArrayList<Member>();
      String sql = "SELECT * FROM MEMBER\r\n"
      		+ "WHERE name LIKE '%'||?||'%'\r\n"
      		+ "AND auth LIKE '%'||?||'%'";
