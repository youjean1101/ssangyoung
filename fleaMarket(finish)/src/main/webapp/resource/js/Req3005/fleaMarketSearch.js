...
function callPageSelector(){
	...
	const rows = document.querySelectorAll('.TotList');// 게시글 row를 전체 선택
	...
	function displayRow(idx){
		...
		//console.log(rowsArray)
		 ...
		 for(ra of rowsArray){
			...
			/*ra.firstElementChild.firstElementChild.firstElementChild.firstElementChild.setAttribute("name","email2")*/
		} 
		  let newRows = rowsArray.slice(start,end);
		for(nr of newRows){
			nr.style.display='';
		/*	nr.firstElementChild.firstElementChild.firstElementChild.firstElementChild.setAttribute("name","email")*/
		}   
